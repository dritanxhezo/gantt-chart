export function DateTimeRenderer(root) {
  ...
  var initSettings = function () {
    if (shadowRoot) {
      ...
      setDateFrom(
        `${this.dateFrom.getFullYear()}-${zeroPad(
          this.dateFrom.getMonth() + 1
        )}-${zeroPad(this.dateFrom.getDate())}`
      );
      setDateTo(
        `${this.dateTo.getFullYear()}-${zeroPad(
          this.dateTo.getMonth() + 1
        )}-${zeroPad(this.dateTo.getDate())}`
      );
    }
  }.bind(this);
  ...
  var initJobs = function () {
    this.jobs.forEach((job) => {
      ...
      var ganttElement = shadowRoot.querySelector(
        `div[data-resource="${job.resource}"][data-date="${date_string}"]`
      );
